...
### 9.7.1 new Promise() 생성자

Promise() 생성자는 항상 new와 함께 사용하며 동기적으로/즉시 호출할 콜백 함수를 전달해야 함  
이 함수에는 다시 프라미스를 귀결 처리할 콜백 2개를 넘기는데 resolve()와 reject()라고 명명하는 것이 보통임

reject()는 그냥 프라미스를 버리지만 resolve()는 넘어온 값을 보고 이룸/버림 중 한 가지로 처리  
즉시값, 프라미스 아닌/데너블 아닌 값이 resolve()에 흘러오면 이 프라미스는 해당 값으로 이루어짐

반면, resolve()에 진짜 프라미스/데너블 값이 전달되면 재귀적으로 풀어보고 결국 그 최종 값이 프라미스의 마지막 귀결/상태가 됨

### 9.7.2 Promise.resolve()와 Promise.reject()

Promise.reject()는 이미 버려진 프라미스를 생성하는 지름길  
따라서 두 프라미스는 본질적으로 동등

Promise.reject()와 마찬가지로 Promise.resolve()는 이미 이루어진 프라미스를 생성하는 용도로 사용  
Promise.resolve()는 데너블 값을 재귀적으로 풀어보고 그 최종 귀결 값이 결국 반환된 프라미스에 해당됨

Promise.resolve()에 진짜 프라미스를 넣으면 아무 일도 하지 않음

### 9.7.3 then()과 catch()

각 프라미스 인스턴스엔 then(), catch() 메서드가 들어있고 프라미스에 이룸/버림 처리기를 등록 할 수 있음  
프라미스가 귀결되면 두 처리기 중 하나만 언제나 비동기적으로 호출됨

then()은 하나 또는 2개의 인자를 받는데 첫 번째는 이룸 콜백, 두 번째는 버림 콜백  
어느 한 쪽을 누락하거나 함수가 아닌 값으로 지정하면 각각 기본 콜백으로 대체됨  
기본 이룸 콜백은 그냥 메시지를 전달하기만 하고 기본 버림 콜백은 단순히 전달받은 에러 사유를 도로 던짐

catch()는 버림 콜백 하나만 받고 이룸 콜백은 기본 이룸 콜백으로 대체

then(), catch()도 새 프라미스를 만들어 반환하므로 프라미스 연쇄 형태로 흐름 제어 표현 가능  
이룸/버림 콜백에서 예외가 발생하면 반환된 프라미스는 버림  
콜백 반환 값이 즉시값, 프라미스 아닌/데너블 아닌 값이면 반환된 플미스의 이룸값으로 지정  
이룸 처리기나 특정 프라미스가 데너블 값을 반환하면 이 값을 풀어 반환된 프라미스의 귀결 값이 됨

### 9.7.4 Promise.all([])과 Promise.race([])

Promise.all([]), Promise.race([])는 프라미스를 생성하여 반환하는 ES6 프라미스 API의 정적 헬퍼 유틸리티  
프라미스 귀결은 전적으로 전달받은 프라미스 배열에 따라 좌우됨

Promise.al([])은 주어진 모든 프라미스들이 이루어져야 메인 프라미스도 이루어지고 단 하나라도 버려지게 되면 메인 반환 프라미스 역시 곧바로 폐기됨  
이루어지면 각 프라미스의 이룸값이 담긴 배열을, 버려지면 처음 버려진 프라미스의 버림 사유를 돌려받음

Promise.race([])는 오직 최초로 귀결된 프라미스만 승자가 되고 그 귀결 값을 반환할 프라미스의 귀결 값으로 삼음
...
**_빈 배열을 Promise.all([])에 주면 바로 이루어지지만 Promise.race([])는 귀결되지 않은 채 프로그램이 멈춘다._**

ES6 프라미스 API는 아주 간단하고 직관적  
기본적인 비동기 케이스라면 대부분 바로 이용 가능, 콜백 지옥에 빠진 코드를 재편성하여 개선하기 위한 출발점으로서 매우 유용

하지만 애플리케이션을 개발하다 보면 종종 프라미스만으로는 해결하기 어려운, 복잡 미묘한 다양한 문제들이 존재

## 9.8 프라미스 한계

### 9.8.1 시퀀스 에러 처리

프라미스 에러는 고려해야 할 점이 있음  
프라미스 연쇄는 구성원들을 한데 모아놓은 사슬에 불과하기 때문에 전체 연쇄를 하나의 '무언가'로 가리킬 개체가 마땅치 않음  
즉, 일어날지 모를 에러를 밖에서는 감지할 방법이 없음

에러 처리기가 없는 프라미스 연쇄에서 에러가 나면 나중에 어딘가에서 감지될 때까지 연쇄를 따라 쭉 하위로 전파함  
그래서 이런 경우라면 연쇄의 마지막 프라미스를 가리키는 레퍼런스만 갖고 있으면 여기에 버림 처리기를 등록하여 전파되어 내려온 에러 처리 가능
...
또 프라미스 연쇄는 각 단계에서 자신의 에러를 감지하여 처리할 방법 자체가 없으니 p에 에러 처리기를 달아놓으면 연쇄 어디에서 에러가 나도 이를 받아 처리 가능

```javascript
p.catch(handleErrors);
```

하지만 연쇄의 어느 단계에서 나름대로 에러 처리를 하면 handleErrors()는 에러를 감지할 방법이 없음

이것은 try...catch에도 기본적으로 존재하는 한계로, 예외가 잡혀도 그냥 묻혀버릴 가능성이 존재

일반적으로 프라미스 연쇄 시퀀스에서 중간 단계를 참조할 레퍼런스가 없기 때문에 정확하게 에러를 솎아낼 에러 처리기를 붙일 수 없음

### 9.8.2 단일값

프라미스는 정의 상 하나의 이룸값, 아니면 하나의 버림 사유를 가짐  
로직이 복잡해지면 발목을 붙들 수 있는 특성

메시지를 여러 개 담아둘 값 감싸미를 만들면 된다고 하지만, 잘 작동은 하겠으나 프라미스 연쇄의 단계마다 메시지를 감싸고 푸는 일은 무척 불편하고 지루할 것임

##### 값을 분할

두 값(x와 y)을 비동기적으로 만들어내는 유틸리티 함수가 있다고 생각해보자

한 개의 프라미스를 통해 값 배열보다 프라미스 배열을 넘기는 형태가 더 좋다고 생각하는가

먼저 x, y를 하나의 array 값으로 감싸서 프라미스 하나로 전달할 필요가 없도록 하는 코드가 더 좋음  
이렇게 해야 차후 x, y의 계산을 별도의 함수로 분리해서 리팩토링하기가 더 쉽고 호출부로 하여금 두 프라미스를 알아서 조정하도록 놔두는 편이 더 깔끔하고 유연함
...
ES6부터는 해체라는 방법 제공  
또한, ES6는 인자도 배열 해체 형식으로 사용 가능

p. 283 참고

### 9.8.3 단일 귀결

프라미스가 단 1회만 귀결된다는 점은 프라미스의 가장 중요한 본질  
비동기 프로그래밍에서는 대부분 값을 한번만 가져옴

하지만, 데이터 이벤트/스트림과 더 밀접한 관련이 있는 비동기 케이스에도 프라미스가 잘 작동할까?  
프라미스에 중요한 추상화를 더하지 않고서는 다수의 귀결 값을 처리하는 상황에 바로 적용하기에는 어려움이 있음

### 9.8.4 타성

콜백이 이미 뿌리깊이 자리잡은 코드라면 계속 같은 스타일을 유지하는 편이 더 쉬움

프라미스 체계는 차원이 달라서 전체적인 코드 접근 방식 자체가 다를 수 밖에 없고 아예 근원부터 다른 경우도 더러 존재

이러한 한계점까지 프라미스가 정답을 주진 않음

Promise.wrap()은 프라미스를 직접 만들지 않고 프라미스를 만드는 함수를 만듦

콜백식 함수를 프라미스-인식형 함수로 감싸는 것을 '리프팅' 또는 '프라미시파잉'이라고 함

### 9.8.5 프라미스는 취소 불가

일단 프라미스를 생성하여 이룸/버림 처리기를 등록하면, 외부에서 프라미스 진행을 멈출 방법은 없음

**_수많은 프라미스 추상화 라이브러리에서 프라미스 취소 기능을 제공하고 있지만 정말 나쁜 생각이다! 개발자 입장에선 처음부터 외부에서 프라미스를 취소할 수 있게 만들어졌더라면 좋지 않을까 싶겠지만 특정한 프라미스의 사용자/감지자가 같은 프라미스를 바라보는 다른 사용자에게까지 영향을 끼칠 수 있다는 점이 문제다. 이는 미랫값의 믿음성 원리에 위배될뿐만 아니라 '원격 작용'이라는 안티패턴을 구현한 것이기도 하다. 프라미스 취소 기능이 꽤나 쓸모있을 것 같겠지만 결국 콜백 지옥과 똑같은 악몽이 재현될 것이다._**

'프라미스 취소'는 더 상위의 프라미스 추상화 수준에서 구현해야 할 기능  
꼼수를 써서 해결해보려 하지 말고 프라미스 추상화 라이브러리를 찾아 쓰는 편이 좋음

사실 프라미스 한 개는 '취소'라는 말이 지칭하는 흐름 제어 체계라고 볼 수 없음

반면, 프라미스가 모여있는 프라미스 연쇄는 흐름 제어를 나타낸 것이 분명하므로 이 추상화 수준에서 취소 기능을 정의하는 건 타당함

### 9.8.6 프라미스 성능

프라미스 연쇄는 처리량이 더 많기 때문에 콜백식 비동기 작업 연쇄보다 속도가 약간 더 느림  
하지만 같은 수준의 믿음성을 보장하기 위해서는 프라미스는 몇 가지 장치만을 추가하면 되는 반면, 콜백에는 많은 코드를 추가해야 함

프라미스가 '모든 것'을 비동기화한다는 단점도 있다고 함  
어느 정도 즉시 완료된 단계들이 여전히 다음 단계의 잡 진행을 지연시킨다는 것

프라미스의 믿음성/조합성 보장은 매우 큰 혜택

프라미스가 약간 느린 건 사실이나 그 대신 믿음성, 예측성, 조합성과 같은 장점을 고루 누릴 수 있음

## 9.9 정리하기

프라미스는 훌륭함  
콜백식 코드에서 문제였던 '제어의 역전' 문제를 해결

그렇다고 프라미스가 콜백을 완전히 없애는 것은 아님  
기존 콜백 코드에서 믿음성/조합성 등을 원할 경우 프라미스를 사용

프라미스 연쇄는 비동기 흐름을 순차적으로 표현하는 더 나은 방법
